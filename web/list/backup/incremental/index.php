...
if (empty($_GET["snapshot"])) {
	exec(DAVID_CMD . "v-list-user-backups-restic $user json", $output, $return_var);
	...
	if (empty($_GET["browse"])) {
		...
		exec(DAVID_CMD . "v-list-user-backup-restic $user $snapshot json", $output, $return_var);
		$data = json_decode(implode("", $output), true);
		...
		$snapshot = quoteshellarg($_GET["snapshot"]);
		exec(DAVID_CMD . "v-list-user-files-restic $user $snapshot $folder", $output, $return_var);
