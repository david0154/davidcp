...
if (
	...
	exec(DAVID_CMD . "v-log-user-logout " . $v_user . " " . $v_session_id, $output, $return_var);
	destroy_sessions();
...
if (isset($_SESSION["user"])) {
	...
	exec(DAVID_CMD . "v-list-user " . quoteshellarg($username) . " json", $output, $return_var);
...
if (!defined("NO_AUTH_REQUIRED")) {
	if (empty($_SESSION["LAST_ACTIVITY"]) || empty($_SESSION["INACTIVE_SESSION_TIMEOUT"])) {
		...
		$v_session_id = quoteshellarg($_SESSION["token"]);
		exec(
			DAVID_CMD . "v-log-user-logout " . $v_user . " " . $v_session_id,
			$output,
			$return_var,
		);
		destroy_sessions();
...
function top_panel($user, $TAB) {
	$command = DAVID_CMD . "v-list-user " . $user . " 'json'";
...
function backendtpl_with_webdomains() {
	exec(DAVID_CMD . "v-list-users json", $output, $return_var);
	...
	foreach ($users as $user => $user_details) {
		exec(
			DAVID_CMD . "v-list-web-domains " . quoteshellarg($user) . " json",
			$output,
			$return_var,
		);
