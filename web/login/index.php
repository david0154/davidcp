...
if (isset($_SESSION["user"])) {
	...
	if ($_SESSION["userContext"] === "admin" && !empty($_GET["loginas"])) {
		...
		if (verify_csrf($_GET)) {
			...
			exec(DAVID_CMD . "v-list-user " . $v_user . " json", $output, $return_var);
			if ($return_var == 0) {
...
function authenticate_user($user, $password, $twofa = "") {
	...
	if (verify_csrf($_POST, true)) {
		...
		$output = "";
		exec(
			DAVID_CMD . "v-get-user-salt " . $v_user . " " . $v_ip . " json",
			$output,
			$return_var,
		);
		...
		if ($return_var > 0) {
			...
			// Check user hash
			exec(
				DAVID_CMD . "v-check-user-hash " . $v_user . " " . $v_hash . " " . $v_ip,
				$output,
				$return_var,
			);
			unset($output);
			...
			if ($return_var > 0) {
				...
				if ($data[$user]["TWOFA"] != "") {
					if (empty($twofa)) {
						...
						exec(
							DAVID_CMD . "v-check-user-2fa " . $v_user . " " . $v_twofa,
							$output,
							$return_var,
						);
						unset($output);
