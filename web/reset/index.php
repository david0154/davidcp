...
if (!empty($_POST["user"]) && empty($_POST["code"])) {
	...
	if ($return_var == 0) {
		...
		unset($output);
		exec(DAVID_CMD . "v-get-user-value " . $v_user . " RKEYEXP", $output, $return_var);
		...
		if ($rkeyexp === null || $rkeyexp < time() - 1) {
			if ($email == $data[$user]["CONTACT"]) {
				...
				fclose($fp);
				exec(
					DAVID_CMD . "v-change-user-rkey " . $v_user . " " . $v_rkey . "",
					$output,
					$return_var,
				);
				unset($output);
		...
		if ($return_var == 0) {
			...
			if (password_verify($_POST["code"], $rkey)) {
				...
				if ($output[0] > time() - 900) {
					...
					fclose($fp);
					exec(
						DAVID_CMD . "v-change-user-password " . $v_user . " " . $v_password,
						$output,
						$return_var,
					);
					unlink($v_password);
