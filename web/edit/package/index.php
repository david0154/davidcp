...
exec(DAVID_CMD . "v-list-user-package " . $v_package . " 'json'", $output, $return_var);
...
// List dns templates
exec(DAVID_CMD . "v-list-dns-templates json", $output, $return_var);
...
if (!empty($_POST["save"])) {
	...
	$tmpfile = tempnam("/tmp/", "dvp_");
	$fp = fopen($tmpfile, "w");
	fwrite($fp, $pkg);
	exec(
		DAVID_CMD . "v-add-user-package " . $tmpfile . " " . $v_package . " yes",
		$output,
		$return_var,
	);
	...
	exec(DAVID_CMD . "v-update-user-package " . $v_package . " 'json'", $output, $return_var);
	check_return_code($return_var, $output);
	unset($output);

	if ($v_package_new != $v_package) {
		exec(
			DAVID_CMD . "v-rename-user-package " . $v_package . " " . $v_package_new,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
