...
// List user
exec(DAVID_CMD . "v-list-user " . quoteshellarg($v_username) . " json", $output, $return_var);
...
// List packages
exec(DAVID_CMD . "v-list-user-packages json", $output, $return_var);
...
// List shells
exec(DAVID_CMD . "v-list-sys-shells json", $output, $return_var);
$shells = json_decode(implode("", $output), true);
unset($output);

//List PHP Versions
// List supported php versions
exec(DAVID_CMD . "v-list-sys-php json", $output, $return_var);
...
if (!empty($_POST["save"])) {
	...
	if (!empty($_POST["v_twofa"]) && empty($v_twofa) && empty($_SESSION["error_msg"])) {
		exec(DAVID_CMD . "v-add-user-2fa " . quoteshellarg($v_username), $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);

		// List user
		exec(
			DAVID_CMD . "v-list-user " . quoteshellarg($v_username) . " json",
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	// Disable twofa
	if (empty($_POST["v_twofa"]) && !empty($v_twofa) && empty($_SESSION["error_msg"])) {
		exec(DAVID_CMD . "v-delete-user-2fa " . quoteshellarg($v_username), $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
		$v_twofa = "";
		$v_qrcode = "";
	...
	if ($_SESSION["userContext"] === "admin") {
		...
		if (
			...
			if (!empty($_POST["v_role"])) {
				...
				exec(
					DAVID_CMD . "v-change-user-role " . quoteshellarg($v_username) . " " . $v_role,
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
	...
	if ($v_language != $_POST["v_language"] && empty($_SESSION["error_msg"])) {
		...
		exec(
			DAVID_CMD . "v-change-user-language " . quoteshellarg($v_username) . " " . $v_language,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if ($v_email != $_POST["v_email"] && empty($_SESSION["error_msg"])) {
		if (!filter_var($_POST["v_email"], FILTER_VALIDATE_EMAIL)) {
			...
			exec(
				DAVID_CMD . "v-change-user-contact " . quoteshellarg($v_username) . " " . $v_email,
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
	...
	if ($v_name != $_POST["v_name"]) {
		if (empty($_POST["v_name"])) {
			...
			exec(
				DAVID_CMD . "v-change-user-name " . quoteshellarg($v_username) . " " . $v_name,
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
