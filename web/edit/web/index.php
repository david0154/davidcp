...
exec(DAVID_CMD . "v-list-web-domains " . $user . " json", $output, $return_var);
...
exec(
	DAVID_CMD . "v-list-web-domain " . $user . " " . quoteshellarg($v_domain) . " json",
	$output,
	$return_var,
);
...
if (!empty($v_ssl)) {
	exec(
		DAVID_CMD . "v-list-web-domain-ssl " . $user . " " . quoteshellarg($v_domain) . " json",
		$output,
		$return_var,
	);
	...
	$v_ssl_dvps = $data[$v_domain]["SSL_DVPS"];
}
...
// List ip addresses
exec(DAVID_CMD . "v-list-user-ips " . $user . " json", $output, $return_var);
...
// List web templates
exec(DAVID_CMD . "v-list-web-templates json", $output, $return_var);
...
if (!empty($_SESSION["PROXY_SYSTEM"])) {
	exec(DAVID_CMD . "v-list-web-templates-proxy json", $output, $return_var);
	$proxy_templates = json_decode(implode("", $output), true);
	unset($output);
}

// List web stat engines
exec(DAVID_CMD . "v-list-web-stats json", $output, $return_var);
...
if (!empty($_POST["save"])) {
	...
	if ($v_ip != $_POST["v_ip"] && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-list-dns-domain " . $user . " " . quoteshellarg($v_domain) . " json",
			$output,
			$return_var,
		);
		unset($output);
	...
	if ($v_ip != $_POST["v_ip"] && empty($_SESSION["error_msg"])) {
		foreach ($valiases as $v_alias) {
			exec(
				DAVID_CMD . "v-list-dns-domain " . $user . " " . quoteshellarg($v_alias) . " json",
				$output,
				$return_var,
			);
			unset($output);
	...
	if ($v_ip != $_POST["v_ip"] && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-list-mail-domain " . $user . " " . quoteshellarg($v_domain) . " json",
			$output,
			$return_var,
		);
		unset($output);
		if ($return_var == 0) {
			exec(
				DAVID_CMD . "v-rebuild-mail-domain " . $user . " " . quoteshellarg($v_domain),
				$output,
				$return_var,
			);
			check_return_code($return_var, $output);
	...
	if (
		...
		if (
			...
			if ($_POST["v_nginx_cache_check"] == "on") {
				...
				exec(
					DAVID_CMD . "v-delete-fastcgi-cache " . $user . " " . quoteshellarg($v_domain),
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"])) {
		...
		foreach ($result as $alias) {
			if (empty($_SESSION["error_msg"]) && !empty($alias)) {
				...
				if (empty($_SESSION["error_msg"])) {
					exec(
						DAVID_CMD . "v-list-dns-domain " . $user . " " . quoteshellarg($v_domain),
						$output,
						$return_var,
					);
					unset($output);
	...
	if (
		...
		exec(
			DAVID_CMD . "v-add-web-domain-ssl-force " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	// Add SSL HSTS
	if (!empty($_POST["v_ssl_dvps"]) && !empty($_POST["v_ssl"]) && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-add-web-domain-ssl-dvps " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (
		...
		exec(
			DAVID_CMD . "v-delete-web-domain-ssl-force " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	// Delete SSL HSTS
	if ($v_ssl_dvps == "yes" && empty($_POST["v_ssl_dvps"]) && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-delete-web-domain-ssl-dvps " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($v_stats) && $_POST["v_stats"] == "none" && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-delete-web-domain-stats " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($v_stats_user) && empty($_POST["v_stats_auth"]) && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-delete-web-domain-stats-user " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($v_redirect) && empty($_POST["v-redirect-checkbox"])) {
		exec(
			DAVID_CMD . "v-delete-web-domain-redirect " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($restart_web) && empty($_SESSION["error_msg"])) {
		exec(DAVID_CMD . "v-restart-web", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
	...
	if (
		...
		exec(DAVID_CMD . "v-restart-proxy", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
	...
	if (!empty($restart_dns) && empty($_SESSION["error_msg"])) {
		exec(DAVID_CMD . "v-restart-dns", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
