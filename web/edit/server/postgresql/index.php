...
if (!empty($_POST["save"])) {
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config"])) {
		...
		fclose($fp);
		exec(
			DAVID_CMD . "v-change-sys-service-config " . $new_conf . " postgresql " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
...
// List config
exec(DAVID_CMD . "v-list-sys-pgsql-config json", $output, $return_var);
...
// Read config
$v_options = shell_exec(DAVID_CMD . "v-open-fs-config " . $v_options_path);
