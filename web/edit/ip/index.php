...
exec(DAVID_CMD . "v-list-sys-ip " . $v_ip . " 'json'", $output, $return_var);
...
// List users
exec(DAVID_CMD . "v-list-sys-users 'json'", $output, $return_var);
...
if (!empty($_POST["save"])) {
	...
	if ($v_ipstatus == "shared" && empty($_POST["v_shared"]) && empty($_SESSION["error_msg"])) {
		exec(DAVID_CMD . "v-change-sys-ip-status " . $v_ip . " 'dedicated'", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
		$v_dedicated = "yes";
	}
	if ($v_ipstatus == "dedicated" && !empty($_POST["v_shared"]) && empty($_SESSION["error_msg"])) {
		exec(DAVID_CMD . "v-change-sys-ip-status " . $v_ip . " 'shared'", $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
		unset($v_dedicated);
	}
	...
	if ($v_owner != $_POST["v_owner"] && empty($_SESSION["error_msg"])) {
		...
		exec(DAVID_CMD . "v-change-sys-ip-owner " . $v_ip . " " . $v_owner, $output, $return_var);
		check_return_code($return_var, $output);
	...
	if ($v_name != $_POST["v_name"] && empty($_SESSION["error_msg"])) {
		...
		exec(DAVID_CMD . "v-change-sys-ip-name " . $v_ip . " " . $v_name, $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
	...
	if ($v_nat != $_POST["v_nat"] && empty($_SESSION["error_msg"])) {
		...
		exec(DAVID_CMD . "v-change-sys-ip-nat " . $v_ip . " " . $v_nat, $output, $return_var);
		check_return_code($return_var, $output);
		unset($output);
	}
