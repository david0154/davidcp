...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Detect if a David CP theme is already applied
    const hasDavidDark = Array.from(allLinks).some(link => /themes\/david\/dark\.css/.test(link.href));
    const hasDavidLight = Array.from(allLinks).some(link => /themes\/david\/light\.css/.test(link.href));

    // Detect any active MaxTheme theme
    const isMaxLight = Array.from(allLinks).some(link => /maxtheme-light-.*\.min\.css/.test(link.href));
    const isMaxDark = Array.from(allLinks).some(link => /maxtheme-dark-.*\.min\.css/.test(link.href));

    // Inject the corresponding David theme if not already loaded
    if (isMaxDark && !hasDavidDark) {
        const darkLink = document.createElement('link');
        darkLink.rel = 'stylesheet';
        darkLink.href = '/css/src/themes/david/dark.css';
        document.head.appendChild(darkLink);
    } else if (isMaxLight && !hasDavidLight) {
        const lightLink = document.createElement('link');
        lightLink.rel = 'stylesheet';
        lightLink.href = '/css/src/themes/david/light.css';
