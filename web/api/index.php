...
function api_legacy(array $request_data) {
	...
	if ($settings["config"]["API"] != "yes") {
		...
		api_error(E_DISABLED, "Error: API Disabled", $dvp_return);
	}
	...
	if ($settings["config"]["API_ALLOWED_IP"] != "allow-all") {
		...
		if (!in_array(get_real_user_ip(), $ip_list)) {
			api_error(E_FORBIDDEN, "Error: IP is not allowed to connect with API", $dvp_return);
		}
	}
	...
	if (empty($request_data["hash"])) {
		...
		if ($request_data["user"] != "$root_user") {
			api_error(E_FORBIDDEN, "Error: authentication failed", $dvp_return);
		}
		$password = $request_data["password"];
		if (!isset($password)) {
			api_error(E_PASSWORD, "Error: authentication failed", $dvp_return);
		}
	...
	$dvp_cmd = trim($request_data["cmd"] ?? "");
	$dvp_cmd_args = [];
	for ($i = 1; $i <= 13; $i++) {
		if (isset($request_data["arg{$i}"])) {
			$dvp_cmd_args["arg{$i}"] = trim($request_data["arg{$i}"]);
		}
	}

	if (empty($dvp_cmd)) {
		api_error(E_INVALID, "Command not provided", $dvp_return);
	} elseif (!preg_match('/^[a-zA-Z0-9_-]+$/', $dvp_cmd)) {
		api_error(E_INVALID, "$dvp_cmd command invalid", $dvp_return);
	}

	// Check command
	if ($dvp_cmd == "v-make-tmp-file") {
		// Used in DNS Cluster
		$fp = fopen("/tmp/" . basename(escapeshellcmd($dvp_cmd_args["arg2"])), "w");
		fwrite($fp, $dvp_cmd_args["arg1"] . "\n");
		fclose($fp);
		$return_var = 0;
	} else {
		// Prepare command
	...
	if (!empty($dvp_return) && $dvp_return == "code") {
		echo $cmd_exit_code;
	} else {
...
function api_connection(array $request_data) {
	...
	if ($api_status == 0) {
		...
		api_error(E_DISABLED, "API has been disabled", $dvp_return);
	}
	...
	if ($settings["config"]["API_ALLOWED_IP"] != "allow-all") {
		...
		if (!in_array($v_real_user_ip, $ip_list) && !in_array("0.0.0.0", $ip_list)) {
			api_error(E_FORBIDDEN, "IP is not allowed to connect with API", $dvp_return);
		}
	}

	// Get POST Params
	$dvp_access_key_id = trim($request_data["access_key"] ?? "");
	$dvp_secret_access_key = trim($request_data["secret_key"] ?? "");
	$dvp_cmd = trim($request_data["cmd"] ?? "");
	$dvp_cmd_args = [];
	for ($i = 1; $i <= 13; $i++) {
		if (isset($request_data["arg{$i}"])) {
	...
	if (empty($dvp_access_key_id) || empty($dvp_secret_access_key)) {
		api_error(E_PASSWORD, "Authentication failed", $dvp_return);
	}
	...
	exec(
		DAVID_CMD .
			...
			" " .
			quoteshellarg($dvp_secret_access_key) .
			" " .
			quoteshellarg($dvp_cmd) .
			" " .
	...
	if ($return_var > 0) {
		//api_error($return_var, "Key $dvp_access_key_id - authentication failed", $dvp_return);
		api_error($return_var, $output, $dvp_return);
	}
	...
	if ($key_user != $root_user && $api_status < 2) {
		api_error(E_API_DISABLED, "API has been disabled", $dvp_return);
	}
	...
	if (
		$key_user != $root_user &&
		$user_arg_position > 0 &&
		$dvp_cmd_args["arg{$user_arg_position}"] != $key_user
	) {
		api_error(
			E_FORBIDDEN,
			"Key $dvp_access_key_id - the \"user\" argument doesn\'t match the key\'s user",
			$dvp_return,
		);
	}
	...
	if ($dvp_cmd == "v-make-tmp-file") {
		$fp = fopen("/tmp/" . basename($dvp_cmd_args["arg2"]), "w");
		fwrite($fp, $dvp_cmd_args["arg1"] . "\n");
		fclose($fp);
		$cmd_exit_code = 0;
	} else {
	...
	header("David-Exit-Code: $cmd_exit_code");
...
if (isset($_POST["access_key"]) || isset($_POST["user"]) || isset($_POST["hash"])) {
	...
	api_error(
		405,
		"Error: data received is null or invalid, check https://davidk.online/docs/server-administration/rest-api.html",
		"",
	);
}
