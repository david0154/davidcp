...
if (!empty($_POST["ok"])) {
	...
	// Gets public IP from 'v-list-user-ips' command (that reads /david/data/ips/ip), precisely from 'NAT' field
	$v_public_ip = $v_ip;
	...
	exec(DAVID_CMD . "v-list-user " . $user . " json", $output, $return_var);
	...
	if ($_POST["v_mail"] == "on" && empty($_SESSION["error_msg"])) {
		exec(
			DAVID_CMD . "v-add-mail-domain " . $user . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
