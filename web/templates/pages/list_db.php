	...
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			<?php if ($read_only !== "true") { ?>
				...
				<?php } ?>
				<?php if (ipUsed()) { ?>
					<a target="_blank" href="https://davidk.online/docs/server-administration/databases.html#why-i-can-t-use-http-ip-phpmyadmin">
						...
						<ul class="units-table-row-actions">
							...
							<?php if ($data[$key]['TYPE'] == 'mysql' && isset($_SESSION['PHPMYADMIN_KEY']) && $_SESSION['PHPMYADMIN_KEY'] != '' && !ipUsed()) { $time = time(); ?>
								<li class="units-table-row-action shortcut-enter" data-key-action="href">
									<a
										class="units-table-row-action-link"
										href="<?= $db_myadmin_link?>david-sso.php?database=<?= $key ?>&user=<?= $user_plain?>&exp=<?= $time?>&david_token=<?=password_hash($key.$user_plain.$_SESSION['user_combined_ip'].$time.$_SESSION['PHPMYADMIN_KEY'], PASSWORD_DEFAULT) ?>"
