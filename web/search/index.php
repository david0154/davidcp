...
if ($_SESSION["userContext"] === "admin" && $_SESSION["look"] == "") {
	if (!empty($_GET["u"])) {
		$user = $u;
		exec(
			DAVID_CMD . "v-search-user-object " . $user . " " . $q . " json",
			$output,
			$return_var,
		);
	} else {
		exec(DAVID_CMD . "v-search-object " . $q . " json", $output, $return_var);
	}
} else {
	exec(DAVID_CMD . "v-search-user-object " . $user . " " . $q . " json", $output, $return_var);
}
