...
#===========================================================================#
#                                                                           #
# David Control Panel - Core Function Library                              #
#                                                                           #
#===========================================================================#
...
if [ -z "$user" ]; then
	if [ -z "$ROOT_USER" ]; then
		...
		source_conf "$DAVID/conf/david.conf" # load config file
	fi
	user="$ROOT_USER"
fi
...
log_event() {
	...
	if [ "$1" -eq 0 ]; then
		echo "$LOG_TIME $2" >> $DAVID/log/system.log
	else
		echo "$LOG_TIME $2 [Error $1]" >> $DAVID/log/error.log
	fi
}
...
log_history() {
	...
	if [ "$log_user" = "system" ]; then
		log=$DAVID/log/activity.log
	else
		if ! $BIN/v-list-user "$log_user" > /dev/null; then
			return $E_NOTEXIST
		fi
		log=$DAVID/data/users/$log_user/history.log
	fi
	touch $log
...
is_package_valid() {
	if [ -z $1 ]; then
		if [ ! -e "$DAVID/data/packages/$package.pkg" ]; then
			check_result "$E_NOTEXIST" "package $package doesn't exist"
		fi
	else
		if [ ! -e "$DAVID/data/packages/$1.pkg" ]; then
...
# Check user backup settings
is_backup_scheduled() {
	if [ -e "$DAVID/data/queue/backup.pipe" ]; then
		check_q=$(grep " $user " $DAVID/data/queue/backup.pipe | grep $1)
		if [ -n "$check_q" ]; then
...
is_object_valid() {
	if [ $2 = 'USER' ]; then
		tstpath="$(readlink -f "$DAVID/data/users/$3")"
		if [ "$(dirname "$tstpath")" != "$(readlink -f "$DAVID/data/users")" ] || [ ! -d "$DAVID/data/users/$3" ]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	elif [ $2 = 'KEY' ]; then
		local key="$(basename "$3")"

		if [[ -z "$key" || ${#key} -lt 16 ]] || [[ ! -f "$DAVID/data/access-keys/${key}" && ! -f "$DAVID/data/access-keys/$key" ]]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	else
		object=$(grep "$2='$3'" $DAVID/data/users/$user/$1.conf)
		if [ -z "$object" ]; then
			arg1=$(basename $1)
...
update_user_value() {
	key="${2//$/}"
	lnr=$(grep -n "^$key='" $DAVID/data/users/$1/user.conf | cut -f 1 -d ':')
	if [ -n "$lnr" ]; then
		sed -i "$lnr d" $DAVID/data/users/$1/user.conf
		sed -i "$lnr i\\$key='${3}'" $DAVID/data/users/$1/user.conf
	fi
}
...
increase_user_value() {
	key="${2//$/}"
	factor="${3-1}"
	conf="$DAVID/data/users/$1/user.conf"
...
check_access_key_secret() {
	...
	if [[ -z "$access_key_id" || ! -f "$DAVID/data/access-keys/${access_key_id}" ]]; then
...
check_access_key_cmd() {
	...
	if [[ "$DEBUG_MODE" = "true" ]]; then
		new_timestamp
		echo "[$date:$time] $1 $2" >> /var/log/david/api.log
	fi
	if [[ -z "$access_key_id" || ! -f "$DAVID/data/access-keys/${access_key_id}" ]]; then
		check_result "$E_FORBIDEN" "Access key $access_key_id doesn't exist"
	fi
	...
	if [[ -z "$cmd" ]]; then
		...
		if [[ -n "$PERMISSIONS" ]]; then
			...
			if [[ -z "$(echo ",${allowed_commands}," | grep ",${dvp_command},")" ]]; then
				check_result "$E_FORBIDEN" "Key $access_key_id don't have permission to run the command $dvp_command"
			fi
		...
		if [[ "$USER" == "$ROOT_USER" ]]; then
			...
			user_arg_position="$(search_command_arg_position "$dvp_command" "USER")"
			if ! [[ "$user_arg_position" =~ ^[0-9]+$ ]]; then
				check_result "$E_FORBIDEN" "Command $dvp_command not found"
			fi
		fi
	fi
}
...
check_david_demo_mode() {
	demo_mode=$(grep DEMO_MODE /usr/local/david/conf/david.conf | cut -d '=' -f2 | sed "s|'||g")
...
is_david_package() {
	...
	if [ "$check" != "true" ]; then
		check_result $E_INVALID "$2 package is not controlled by davidcp"
	fi
}
...
change_sys_value() {
	check_ckey=$(grep "^$1='" "$DAVID/conf/david.conf")
	if [ -z "$check_ckey" ]; then
		echo "$1='$2'" >> "$DAVID/conf/david.conf"
	else
		sed -i "s|^$1=.*|$1='$2'|g" "$DAVID/conf/david.conf"
	fi
}
...
is_key_permissions_format_valid() {
	...
	while IFS=',' read -ra permissions_arr; do
		for permission in "${permissions_arr[@]}"; do
			...
			if [[ ! -e "$DAVID/data/api/$permission" ]]; then
				check_result "$E_NOTEXIST" "API $permission doesn't exist"
			fi

			source_conf "$DAVID/data/api/$permission"
...
# Get the position of an argument by name in a david command using the command's documentation comment.
#
# Return:
# * 0:   It doesn't have the argument;
# * 1-9: The position of the argument in the command.
search_command_arg_position() {
	local dvp_command="$(basename "$1")"
	local arg_name="$2"

	local command_path="$BIN/$dvp_command"
	if [[ -z "$dvp_command" || ! -e "$command_path" ]]; then
		echo "-1"
		return
	fi
