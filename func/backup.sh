...
#===========================================================================#
#                                                                           #
# David Control Panel - Backup Function Library                            #
#                                                                           #
#===========================================================================#
...
local_backup() {
	...
	chown "davidweb":"$user" $BACKUP/$user.$backup_new_date.tar
	localbackup='yes'
...
ftp_backup() {
	# Checking config
	if [ ! -e "$DAVID/conf/ftp.backup.conf" ]; then
	...
	# Parse config
	source_conf "$DAVID/conf/ftp.backup.conf"

	# Set default port
	if [ -z "$(grep 'PORT=' $DAVID/conf/ftp.backup.conf)" ]; then
		PORT='21'
	fi
...
sftp_download() {
	source_conf "$DAVID/conf/sftp.backup.conf"
	if [ -z "$PORT" ]; then
		PORT='22'
	fi
	cd $BACKUP
...
sftp_backup() {
	# Checking config
	if [ ! -e "$DAVID/conf/sftp.backup.conf" ]; then
	...
	# Set default port
	if [ -z "$(grep 'PORT=' $DAVID/conf/sftp.backup.conf)" ]; then
		PORT='22'
	fi
...
b2_backup() {
	...
	source_conf "$DAVID/conf/b2.backup.conf"
...
rclone_backup() {
	...
	source_conf "$DAVID/conf/rclone.backup.conf"
