...
#===========================================================================#
#                                                                           #
# David Control Panel - Upgrade Function Library                           #
#                                                                           #
#===========================================================================#
...
upgrade_health_check() {
	...
	# Perform basic health check against david.conf to ensure that
	...
	if [ -z "$VERSION" ]; then
		...
		echo
		echo "[ ! ] Unable to detect installed version of David Control Panel."
...
upgrade_welcome_message() {
	...
	echo "                                                                              "
	echo "                    David Control Panel Software Update                      "
...
upgrade_welcome_message_log() {
	echo "============================================================================="
	echo "David Control Panel Software Update Log"
	echo "============================================================================="
	echo
...
upgrade_complete_message() {
	...
	echo "https://github.com/davidcp/davidcp/blob/release/CHANGELOG.md               "
	echo
	echo "We hope that you enjoy using this version of David Control Panel,           "
...
upgrade_get_version() {
	# Retrieve new version number for David Control Panel from .deb package
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		echo "$HOSTNAME has been upgraded from David Control Panel v$VERSION to v${new_version}." >> $message_tmp_file
...
upgrade_init_logging() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Development build for testing purposes only. Report bugs at https://github.com/davidcp/davidcp/issues/."
	fi
	if [[ "$new_version" =~ "beta" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Beta release. Please report bugs at https://github.com/davidcp/davidcp/issues/."
	fi
}
...
upgrade_refresh_config() {
	source_conf "/usr/local/david/conf/david.conf"
}
	...
	if [ -n "$(echo $DB_SYSTEM | grep -w 'mysql')" ]; then
		...
		if version_ge "$pma_version" "$pma_v"; then
			...
			if [ -e /var/lib/phpmyadmin/blowfish_secret.inc.php ]; then
				chown root:davidmail /var/lib/phpmyadmin/blowfish_secret.inc.php
				chmod 0640 /var/lib/phpmyadmin/blowfish_secret.inc.php
			fi
			chown davidmail:davidmail /usr/share/phpmyadmin/tmp
...
upgrade_roundcube() {
	if [ -n "$(echo "$WEBMAIL_SYSTEM" | grep -w 'roundcube')" ]; then
		if [ -d "/usr/share/roundcube" ]; then
			...
			echo "      To upgrade to the latest version of Roundcube directly from upstream, from please run the command migrate_roundcube.sh located in: /usr/local/david/install/upgrade/manual/"
		else
...
upgrade_dependencies() {
	echo "[ ! ] Update David PHP dependencies..."
...
upgrade_restart_services() {
	...
	# Always restart the David Control Panel service
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - david"
	fi
