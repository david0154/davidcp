...
mysql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $DAVID/conf/mysql.conf)
...
psql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $DAVID/conf/pgsql.conf)
	parse_object_kv_list "$host_str"
...
is_charset_valid() {
	host_str=$(grep "HOST='$host'" $DAVID/conf/$type.conf)
	parse_object_kv_list "$host_str"
...
increase_dbhost_values() {
	...
	sed -i "s/$old_dbbases/$new_dbbases/g" $DAVID/conf/$type.conf
	sed -i "s/$old_users/$new_users/g" $DAVID/conf/$type.conf
}
...
is_dbhost_new() {
	if [ -e "$DAVID/conf/$type.conf" ]; then
		check_host=$(grep "HOST='$host'" $DAVID/conf/$type.conf)
		if [ "$check_host" ]; then
