...
#===========================================================================#
#                                                                           #
# David Control Panel - IP/Network Function Library                        #
#                                                                           #
#===========================================================================#
...
# Check ip ownership
is_ip_owner() {
	owner=$(grep 'OWNER=' $DAVID/data/ips/$ip | cut -f 2 -d \')
	if [ "$owner" != "$user" ]; then
...
# Get ip name
get_ip_alias() {
	ip_name=$(grep "NAME=" $DAVID/data/ips/$local_ip | cut -f 2 -d \')
	if [ -n "$ip_name" ]; then
		echo "${1//./-}.$ip_name"
	fi
}
...
increase_ip_value() {
	...
	usr_key='U_SYS_USERS'
	current_web=$(grep "$web_key=" $DAVID/data/ips/$sip | cut -f 2 -d \')
	current_usr=$(grep "$usr_key=" $DAVID/data/ips/$sip | cut -f 2 -d \')
...
# Get real ip address
get_real_ip() {
	if [ -e "$DAVID/data/ips/$1" ]; then
		echo "$1"
	else
		nat=$(grep -H "^NAT='$1'" $DAVID/data/ips/* | head -n1)
		if [ -n "$nat" ]; then
...
# Get user ips
get_user_ips() {
	dedicated=$(grep -H "OWNER='$user'" $DAVID/data/ips/*)
	dedicated=$(echo "$dedicated" | cut -f 1 -d : | sed 's=.*/==' | grep -E ${REGEX_IPV4})
	shared=$(grep -H -A1 "OWNER='$ROOT_USER'" $DAVID/data/ips/* | grep shared)
...
get_user_ip() {
	...
	local_ip=$ip
	nat=$(grep "^NAT" $DAVID/data/ips/$ip | cut -f 2 -d \')
	if [ -n "$nat" ]; then
		ip=$nat
	fi
}
...
is_ip_valid() {
	local_ip="$1"
	if [ ! -e "$DAVID/data/ips/$1" ]; then
		nat=$(grep -H "^NAT='$1'" $DAVID/data/ips/*)
		if [ -z "$nat" ]; then
	...
	if [ -n "$2" ]; then
		if [ -z "$nat" ]; then
			ip_data=$(cat $DAVID/data/ips/$1)
		else
			ip_data=$(cat $DAVID/data/ips/$nat)
		fi
