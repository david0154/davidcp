...
function setup() {
    ...
    if [ $BATS_TEST_NUMBER = 1 ]; then
        ...
        echo 'pgdatabase=test5290_database' >> /tmp/david-test-env.sh
        echo 'pgdbuser=test5290_dbuser' >> /tmp/david-test-env.sh
    fi
...
@test "User: Add new user Failed 1" {
	run v-add-user 'jäap' $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}
@test "User: Add new user Failed 2" {
	run v-add-user 'ëaap' $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 3" {
	run v-add-user 'jaaẞ'  $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 4" {
	run v-add-user '1234'  $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 5" {
	run v-add-user '1aap'  $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 6" {
	run v-add-user 'ib_Buffer'  $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: The user name'
}

@test "User: Add new user Failed 7" {
	run v-add-user 'hello.com'  $user $user@davidcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}
...
@test "User: Add new user Success 1" {
	run v-add-user 'jaap01'  $user $user@davidcp2.com default "Super Test"
	assert_success
	refute_output
}
...
@test "User: Add new user Success 2" {
	run v-add-user 'buffer'  $user $user@davidcp2.com default "Super Test"
	assert_success
	refute_output
}
...
@test "Ip: Add new ip on first interface" {
    ...
    # Save initial state
    echo "interface=${interface}" >> /tmp/david-test-env.sh
    [ -f "$a2_rpaf" ]     && file_hash1=$(cat $a2_rpaf     |md5sum |cut -d" " -f1) && echo "a2_rpaf_hash='${file_hash1}'"     >> /tmp/david-test-env.sh
    [ -f "$a2_remoteip" ] && file_hash2=$(cat $a2_remoteip |md5sum |cut -d" " -f1) && echo "a2_remoteip_hash='${file_hash2}'" >> /tmp/david-test-env.sh
...
@test "Ip: [Ubuntu] Netplan file updated" {
   ...
   assert_file_exist /etc/netplan/60-david.yaml

   # also check if file contains the newly added ip
   assert_file_contains /etc/netplan/60-david.yaml "$ip"
}
...
@test "WEB: Add web domain" {
    ...
    echo -e "<?php\necho 'David Test:'.(4*3);" > $HOMEDIR/$user/web/$domain/public_html/php-test.php
    validate_web_domain $user $domain 'David Test:12' 'php-test.php'
...
@test "WEB: Generate Self signed certificate" {
    ssl=$(v-generate-ssl-cert "$domain" "info@$domain" US CA "Orange County" davidcp IT "mail.$domain" | tail -n1 | awk '{print $2}')
    echo $ssl;
...
@test "WEB: Generate Self signed certificate ASCII idn-tést.eu" {
    run v-generate-ssl-cert "xn--idn-tst-fya.eu" "info@xn--idn-tst-fya.eu" US CA "Orange County" davidcp IT "mail.xn--idn-tst-fya.eu"
    assert_success
}
...
@test "WEB: Generate Self signed certificate ASCII bløst.рф" {
    run v-generate-ssl-cert "xn--blst-hra.xn--p1ai" "info@xn--blst-hra.xn--p1ai" US CA "Orange County" davidcp IT "mail.xn--blst-hra.xn--p1ai"
    assert_success
}
...
@test "Multiphp: Change backend version - PHP v5.6" {
    ...
    echo -e "<?php\necho 'david-multiphptest:'.PHP_MAJOR_VERSION.'.'.PHP_MINOR_VERSION;" > "$HOMEDIR/$user/web/$multi_domain/public_html/php-test.php"
    validate_web_domain $user $multi_domain "david-multiphptest:$test_phpver" 'php-test.php'
    rm "$HOMEDIR/$user/web/$multi_domain/public_html/php-test.php"
}
...
@test "DNS: Add domain record MX" {
    ...
    assert_file_contains "$HOMEDIR/$user/conf/dns/${domain}.db" "mx.david.com."
...
@test "MAIL: Add account alias" {
	run v-add-mail-account-alias $user $domain test davidcprocks
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/aliases "davidcprocks@$domain"
	refute_output
}

@test "MAIL: Add account alias 2" {
	run v-add-mail-account-alias $user $domain test davidcprocks2
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/aliases "davidcprocks2@$domain"
	refute_output
}

@test "MAIL: Add account alias 3" {
	run v-add-mail-account-alias $user $domain test davidcp
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/aliases "davidcp@$domain"
	refute_output
}

@test "MAIL: Add account 3" {
	run v-add-mail-account $user $domain david "$userpass2"
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/limits "david@$domain"
	refute_output
}

@test "MAIL: Add account 4" {
	run v-add-mail-account $user $domain davidrocks3 "$userpass2"
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/limits "davidrocks3@$domain"
	refute_output
}
...
@test "MAIL: Add account alias Invalid length" {
	run v-add-mail-account-alias $user $domain test 'davidcp-really-rocks-but-i-want-to-have-feature-xyz-and-i-want-it-now'
	assert_failure $E_INVALID
}
...
@test "MAIL: Add account alias Invalid 2" {
	run v-add-mail-account-alias $user $domain test 'david@test'
	assert_failure $E_INVALID
}
...
@test "Package: Update new Package" {
    sed -i "s/BANDWIDTH='unlimited'/BANDWIDTH='100'/g" /tmp/package
    run v-add-user-package /tmp/package davidtest yes
    assert_success
    refute_output
}
...
@test "Package: Copy package Not Exists" {
  run v-copy-user-package daviddoesnotexists davidtest2
  assert_failure $E_NOTEXIST
}

@test "Package: Copy package" {
  run v-copy-user-package davidtest davidtest2
  assert_success
  refute_output
}
...
@test "Package: Delete package" {
    run v-delete-user-package davidtest
    run v-delete-user-package davidtest2
    rm /tmp/package
    assert_success
    refute_output
