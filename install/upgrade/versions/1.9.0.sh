...
# Check if davidweb exists
if [ -z "$(grep ^davidweb: /etc/passwd)" ]; then
...
if [ ! -f "/var/spool/cron/crontabs/davidweb" ]; then
...
$BIN/v-add-user-notification 'admin' 'David security has been upgraded' ' A new user "davidweb" has been created and is used for login. Make sure other David packages are updated as well otherwise the system may not work as expected.'
add_upgrade_message 'Security has been upgraded, A new user "davidweb" has been created and is used for login. Make sure other David packages are updated as well otherwise the system may not work as expected.'
