...
# David Control Panel upgrade script for target version 1.5.11
...
PORT=$(cat $DAVID/nginx/conf/nginx.conf | grep "listen" | sed 's/[^0-9]*//g')

if [ "$PORT" != "8083" ]; then
	# Update F2B chains config
	if [ -f "$DAVID/data/firewall/chains.conf" ]; then
		# Update value in chains.conf
		sed -i "s/PORT='8083'/PORT='$PORT'/g" $DAVID/data/firewall/chains.conf
	fi

	# Restart services
	if [ -n "$FIREWALL_SYSTEM" ] && [ "$FIREWALL_SYSTEM" != no ]; then
		$DAVID/bin/v-stop-firewall
