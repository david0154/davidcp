...
if [ -f /etc/nginx/nginx.conf ]; then
	...
	sed -i -E 's/(.*keepalive_requests\s{1,})10000;/\11000;/' /etc/nginx/nginx.conf /usr/local/david/nginx/conf/nginx.conf
fi

# Fix security issue wit FPM pools
if [ -z "$(grep ^davidmail: /etc/passwd)" ]; then
	echo "[ * ] Limit permissions www.conf and dummy.conf"
	/usr/sbin/useradd "davidmail" -c "$email" --no-create-home

	sed -i "s/user = www-data/user = davidmail/g" /etc/php/*/fpm/pool.d/www.conf
