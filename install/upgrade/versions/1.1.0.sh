...
# David Control Panel upgrade script for target version 1.1.0
...
if [ -e $DAVID/data/users/history.log ]; then
	rm -f $DAVID/data/users/history.log
fi
...
if [ -e "/etc/cron.d/david-sftp" ]; then
	if ! cat /etc/cron.d/david-sftp | grep -q 'root'; then
		echo "@reboot root /usr/local/david/bin/v-add-sys-sftp-jail" > /etc/cron.d/david-sftp
	fi
fi
...
if [ -e "$DAVID/data/templates/dns/office365.tpl" ]; then
	echo "[ * ] Updating DNS template for Office 365..."
	cp -f $DAVID/install/deb/templates/dns/office365.tpl $DAVID/data/templates/dns/office365.tpl
fi
...
for user in $($DAVID/bin/v-list-users plain | cut -f1); do
	for domain in $($DAVID/bin/v-list-web-domains $user plain | cut -f1); do
		chown root:$user /var/log/$WEB_SYSTEM/domains/$domain.* > /dev/null 2>&1
		for sub_domain in $($DAVID/bin/v-list-web-domain $user $domain plain | cut -f7 | tr ',' '\n'); do
...
if [ "$IMAP_SYSTEM" = "dovecot" ]; then
	...
	if [ -e /etc/dovecot/conf.d/20-pop3.conf ]; then
		cp -f $DAVID/install/deb/dovecot/conf.d/20-pop3.conf /etc/dovecot/conf.d/20-pop3.conf
	fi
	if [ -e /etc/dovecot/conf.d/20-imap.conf ]; then
		cp -f $DAVID/install/deb/dovecot/conf.d/20-imap.conf /etc/dovecot/conf.d/20-imap.conf
	fi
	if [ -e /etc/dovecot/conf.d/90-quota.conf ]; then
		cp -f $DAVID/install/deb/dovecot/conf.d/90-quota.conf /etc/dovecot/conf.d/90-quota.conf
	fi
fi
