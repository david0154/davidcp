...
# David Control Panel upgrade script for target version 1.00.0-190618
...
# Make sure non-admin users belong to correct David group
for user in $($BIN/v-list-users plain | cut -f1); do
	if [ "$user" != "admin" ]; then
		...
		for ftp_user in $(cat /etc/passwd | grep -v "^$user:" | grep "^$user.*:$uid:$uid:" | cut -d ":" -f1); do
			usermod -a -G "david-users" "$ftp_user"
		done
	fi
