...
# David Control Panel upgrade script for target version 1.8.11
...
if [ -d "/var/lib/snappymail" ]; then
	chown davidmail:davidmail /var/lib/snappymail
	chown davidmail:davidmail /etc/snappymail
fi
...
if [ -d "$RC_LOG" ]; then
	chown -R davidmail:www-data "$RC_LOG"
fi
