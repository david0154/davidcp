...
# David Control Panel upgrade script for target version 0.9.8-28
...
if [ -f /usr/local/david/data/firewall/ports.conf ]; then
	rm -f /usr/local/david/data/firewall/ports.conf
fi

# Move clamav to proper location - https://goo.gl/zNuM11
if [ ! -d /usr/local/david/web/edit/server/clamav-daemon ]; then
	mv /usr/local/david/web/edit/server/clamd /usr/local/web/edit/server/clamav-daemon
fi
