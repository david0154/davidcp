...
echo "To remove phpMyAdmin you will need use the root password. Password can be found in /usr/local/david/conf/mysql.conf"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	mkdir -p /root/dvp_backup_man/phmyadmin
	cp -r /etc/phpmyadmin/* /root/dvp_backup_man/phmyadmin

	mkdir -p /root/dvp_backup_man/var_phmyadmin
	cp -r /var/lib/phpmyadmin/* /root/dvp_backup_man/var_phmyadmin
	...
	if [ "$WEB_SYSTEM" == "apache2" ]; then
		cp -f $DAVID_INSTALL_DIR/pma/apache.conf /etc/phpmyadmin/
...
MYSQL_PMA1
	...
	mysql -uroot < $DAVID_INSTALL_DIR/phpmyadmin/create_tables.sql
