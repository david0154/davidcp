...
$dist_config["frontend_config"]["app_name"] = "File Manager - David Control Panel";
...
// Update list of languages when new language is added on David or Filegator side
switch ($lang) {
	case "es":
...
$dist_config["services"]["Filegator\Services\Storage\Filesystem"]["config"][
	...
	if (!empty($_SESSION["INACTIVE_SESSION_TIMEOUT"])) {
		if ($_SESSION["INACTIVE_SESSION_TIMEOUT"] * 60 + $_SESSION["LAST_ACTIVITY"] < time()) {
			...
			exec(
				"/usr/local/david/bin/v-log-user-logout " . $v_user . " " . $v_session_id,
				$output,
				$return_var,
			);
			unset($_SESSION);
	...
	if (!file_exists("/home/" . basename($v_user) . "/.ssh/dvp-filemanager-key")) {
		exec(
			"sudo /usr/local/david/bin/v-add-user-sftp-key " .
	...
	if (!isset($_SESSION["SFTP_PORT"])) {
		exec("sudo /usr/local/david/bin/v-list-sys-sshd-port json", $output, $result);
	...
	return new \League\Flysystem\Sftp\SftpAdapter([
		...
		"privateKey" => "/home/" . basename($v_user) . "/.ssh/dvp-filemanager-key",
		"root" => $root,
		"timeout" => 10,
		"directoryPerm" => 0755,
	]);
...
$dist_config["services"]["Filegator\Services\View\ViewInterface"]["config"] = [
	"add_to_head" => '
	<link rel="stylesheet" href="/fm/css/dvp-custom.css">
    <style>
        .logo {
            width: 46px;
        }
