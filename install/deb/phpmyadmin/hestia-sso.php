...
/* David way to enable support for SSO to PHPmyAdmin */
...
define("API_DAVID_PORT", "%API_DAVID_PORT%");
...
class Hestia_API {
	...
	public function __construct() {
		$this->hostname = "https://" . API_HOST_NAME . ":" . API_DAVID_PORT . "/api/";
		$this->key = API_KEY;
...
if (!empty($_GET)) {
	if (isset($_GET["logout"])) {
		$api->delete_temp_user(
			$_SESSION["DAVID_sso_database"],
			$_SESSION["DAVID_sso_user"],
			$_SESSION["PMA_single_signon_user"],
			$_SESSION["DAVID_sso_host"],
		);
		//remove session
		session_invalid();
	} else {
		if (isset($_GET["user"]) && isset($_GET["david_token"])) {
			$database = $_GET["database"];
			$user = $_GET["user"];
			$host = "localhost";
			$token = $_GET["david_token"];
			...
			if ($time + 60 > time()) {
				...
				if ($data) {
					...
					$_SESSION["DAVID_sso_user"] = $user;
					$_SESSION["DAVID_sso_database"] = $database;
					$_SESSION["DAVID_sso_host"] = $host;
