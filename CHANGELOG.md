...
- Backups change owner files to davidweb (#4779)
...
- Add `david-mail` to `david-users` group and create `david-users` group on new install #4540 #4531
...
- Update david-nginx to 1.27.3
- Update david-php to 8.3.16
...
- Restrict davidmail file permissions (todo should probably have a link here)
...
- Fixed a few bugs due to the permissions changes with davidmail user
- Updated v-list-sys-users to fix issues with new davidmail user
...
- Updated dependency davidcp/phpquoteshellarg to v1.0.1 (#3827)
...
- Increased DVPS max-age to 31536000 (#3762)
- Add prompt to `v-update-sys-david-git` to install NodeJS if not present (#3779)
...
- Fixed david-nginx not loading with custom port
...
- Upload davidcp.pot file directly to Crowdin (#3702)
...
- Update david-php to 8.2.7
- Update david-nginx to 1.25.1
...
- Update david-php to 8.2.5
- Update david-nginx to 1.23.4
...
- davidcp 1.7.2 fixes an issue with downloading certificates from Let's Encrypt due to the implementation of asynchronous requests which will go live on 10th April 2023. Please update your server before this date to ensure compatibility with Let's Encrypt.
...
- Fixed an issue with new david-zone sync and servers behind NAT or with multiple IPs (#3388 #3396)
...
- Update supported message dvp-install.sh (#3377 @shizualand)
...
- Set the correct conflicts for david-php on Ubuntu 22.04
...
A bug in v-update-sys-david caused auto update to be not working. Please run: `apt update && apt upgrade`
...
- Fixed an issue in auto update script for davidcp (#2967)
...
- Update david-nginx to 1.23.2
- Update david-php to 8.1.12
...
- Fix issue in is_david_package (#2889)
...
- Update david-nginx to 1.23.1
- Update david-php to 8.1.9
...
- Fixed an issue where custom ports where not saved on restart fail2ban service making David login screen vulnerable for brute force
...
- Update david-php to 8.1.8
  - Update disable_functions list php.ini for david-php (#2746, #2741)
...
- Add hooks to david-nginx and david-php (#2440)
...
- Fixed an issue in configure-server-smtp.sh by loading /etc/davidcp/david.conf (#2488)
...
- Update david-nginx to 1.22.0
  - Update OpenSSL to 3.0.3
  - Update zlib to 1.2.12
  - Update PCRE to 10.40
- Update david-php to 8.1.7
...
- Fixed an issue where David port change did not update chain for fail2ban (#2465)
...
- Fixed an issue where MariaDB installations were broken when performing a clean install of davidcp v1.5.9 (#2452 | 2446)
...
```bash
rm /usr/share/keyrings/david-keyring.gpg
mkdir /root/.gnupg/
gpg --no-default-keyring --keyring /usr/share/keyrings/david-keyring.gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys A189E93654F0B0E5
```
...
- Allow setting default web install templates for Quick install (#2344) (<https://github.com/davidcp/david-quick-install>)
...
- Remove error_reporting(null) and allow all errors to be logged in /var/log/david/nginx-error.log (#2365)
...
- Limit access openbase dir david-php
- Fixed an issue where an email was send after install nginx command not found (#2328)

### Dependencies

- Update PCRE 8.84 to PCRE2 10.39 for david-nginx package
...
- Improve Update process David and allow versions to decide a a rebuild is required
...
- Added support for openssl in david-php
- Use david-php for installing dependencies to solve issue user configurations (david-php 7.4.22 required)
...
- Synced up translations with davidcp (IT, PL, RU, SK and ZN-CN updated)
...
- Add support for automated testing for davidcp code with @drone
- Add support for SMTP server for internal email #1988 @Myself5 / #1165

### Bugfixes

- Updated jQuery and jQuery UI to the latest version due to a vulnerability in jQuery. @dependabot
- Resolve issue with double ENFORCE_SUBDOMAIN_OWNERSHIP keys in david.conf
...
- Restore of backup did not rebuild the "Forced SSL" and "DVPS" config on new account #1862
- Keep changes made by /install/upgrade/manual/install_awstats_geopip.sh on update davidcp (via Discord)
...
- Fixed an issue where plugin-david-change-pasword did not change the port on v-change-sys-port (Rainloop) #1895
- Fixed an issue where HELO message was not set / error was created on NAT IP

## [1.4.2] - Service release

- **NOTE:** During the 1.4.1 / 1.4.0 release we have introduced a bug for Ubuntu 20.04 and 18.04 users with multiple network ports on the server. This release will solve the problems caused by this bug! If you are unable to download the David packages via apt. Run the following command via CLI or SSH as root
...
- After first reboot of David will try do 1 attempt to request / generate a valid Lets encrypt certificate
...
- Fixed vulnerability in v-update-sys-david (thanks **@numanturle**)
...
- Fixed an issue where the api was enabled after an update of davidcp
...
- Fixed an issue where updates for `david-php` were incorrectly being marked as out-of-date in the UI due to a change in our servicing and package versioning scheme.
...
- Prevent ability to change the password of a non-David user account. Thanks to **Alexandre Zanni**!
...
- Implement a validation function to verify the correct version in david.conf prior to installation.
...
- Fixed an issue where HTTP-to-HTTPS redirection and HTTP Strict Transport Security (DVPS) events were not shown in the user history log.
...
- Improved the functionality of `v-update-sys-david-git` to allow user to specify GitHub repository and whether to build only core package or core and dependencies.
...
- Updated installer to use Ubuntu key server for David APT repository.
...
- Added read-only/demo mode - DEMO_MODE must be set to yes in david.conf to enable.
...
- Reworked Let's Encrypt ACME staging to use David code standards.
...
- Enabled DVPS and force SSL on v-add-letsencrypt-host.
...
- Fixed potential upgrade issue when using v-update-sys-david-git.
...
- Rework Let's Encrypt ACME staging to use david conform standard.
...
- v-sys-update-david-git: Added the ability to update using Git from the command line.
- Implemented support for SFTP chroot jails.
- A newly redesigned user interface which features:
  - A softer color palette which better matches the David Control Panel logo colors.
...
- v-change-web-domain-dvps: Enabled support for HTTP Strict Transport Security (DVPS) on SSL.
