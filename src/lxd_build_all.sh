...
#
# Script for preparing lxd environment and building David packages for all supported distros
...
function setup_container() {
	...
	lxc config device add ${containername} debdir disk path=/opt/davidcp source=${__DIR__}/build/${containername}
}
...
for osdef in "${oslist[@]}"; do
	...
	for osver in $osversions; do
		...
		containername="dvp-${osname}-${osver/\./}"
		container_ip=""
		...
		if [ "$cmd" = '--background' ]; then
			...
			lxc exec $containername -- /opt/davidcp/lxd_compile.sh $branch > /dev/null 2>&1 &
		else
			lxc exec $containername -- /opt/davidcp/lxd_compile.sh $branch
		fi
