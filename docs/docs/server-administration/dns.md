...
## Host DNS for your domain on David

Pre-requisites

These steps require that you configure the DNS servers of your domain to use your David servers.
...
1. On your David master, [create a DNS Zone](../user-guide/dns#adding-a-dns-zone) with the **child-ns** template
2. On your domain registrar panel, set the name servers of the domain to the David servers
...
A Master server is where DNS zones are created, and a Slave server recieves the zone via the API. David can be configured as Master <-> Master or Master -> Slave. With a Master <-> Master configuration, each Master is also a Slave, so it could be considered as Master/Slave <-> Master/Slave.
...
### Master <-> Master DNS cluster (Default setup) with the David API

::: warning
This method does not support DNSSEC!
:::

1. Create a new user on each David server that will act as a “Slave”. Make sure it uses the username of "dns-cluster" or has the role `dns-cluster`
...
### Master -> Slave DNS cluster with the David API

::: info
It doesn't work if you try to sync via local network! See [Issue](https://github.com/davidcp/davidcp/issues/4295) Make sure to use the public ip addresses
:::
...
5. Edit `/usr/local/david/conf/david.conf`, change `DNS_CLUSTER_SYSTEM='david'` to `DNS_CLUSTER_SYSTEM='david-zone'`.
...
1. On the **Master** server, open `/usr/local/david/conf/david.conf`, change `DNS_CLUSTER_SYSTEM='david'` to `DNS_CLUSTER_SYSTEM='david-zone'`.
...
1. On **Master** and **Slave** servers, open `/usr/local/david/conf/david.conf`, change `DNS_CLUSTER_SYSTEM='david'` to `DNS_CLUSTER_SYSTEM='david-zone'`.
...
::: warning
DNSSEC can’t be used when David Cluster is active as Master <-> Master
:::
...
::: danger
Removing or disabling the private key in David will make the domain inaccessble.
:::
...
```bash
/usr/local/david/func/remote.sh: line 43: return: Error:: numeric argument required
