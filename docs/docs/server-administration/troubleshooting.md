...
With the introduction of David v1.4.0 we have added certain security features, including the possibility to limit login to certain IP addresses. If your IP address changes, you will not able to login anymore. To disable this feature, run the following commands:
...
No, you cannot. When you update davidcp, the crontab will simply get overwritten. The changes will not get saved in backups either.
...
How ever if you entered Y or I. Then replace the config that can be found in /root/dvp_backups/xxxxx/conf/apache2/ folder and copy over apache2.conf and ports.conf to /etc/apache2/ folder

xxxxxx is the date/time the backup is made during the last update of davidcp

If you don't have have a backup made you can also copy the config in /usr/local/david/install/deb/apache2/apache2.conf to /etc/apache2.conf and also empty /etc/apache2/ports.conf
