...
::: danger
Before upgrading your operating system make sure to make a backup! We do not provide support for broken operating system installations. We only provide this page for information about David issues that may come up while upgrading.
:::
...
Once a backup has been made, update David to the last supported version:
...
cp -f /usr/local/david/install/deb/exim/exim4.conf.4.95.template /etc/exim4/exim4.conf.template
```
...
```bash
rm -f /etc/exim4/exim4.conf.template
cp -f /usr/local/david/install/deb/exim/exim4.conf.4.94.template /etc/exim4/exim4.conf.template
```

### ProFTPD

Comment out [line 29](https://github.com/davidcp/davidcp/blob/1ff8a4e5207aae1e241954a83b7e8070bcdca788/install/deb/proftpd/proftpd.conf#L29) in `/etc/profpd/prodtpd.conf`.
