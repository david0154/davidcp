...
::: warning
After every edit or update the firewall, David will clear the current iptables unless the rules are added via David and [custom script](#how-can-i-customize-iptables-rules).
:::
...
6. Select the data source by entering one of the following:
   ...
   - Script (with `chmod 755`): `/usr/local/david/install/deb/firewall/ipset/blacklist.sh`
   - File: `file:/location/of/file`
   - You can also use one of David’s included sources.
...
David supports setting custom rules, chains or flags, etc. using script.

Script must be here: `/usr/local/david/data/firewall/custom.sh`

1. Create custom.sh: `touch /usr/local/david/data/firewall/custom.sh`
2. Make it executable: `chmod +x /usr/local/david/data/firewall/custom.sh`
